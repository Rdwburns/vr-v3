...
function switchCountries(home, destination, destinationUrl){
    ...
    document.getElementById('modal-text').innerHTML = `<p>You are currently on our ${home} site, but you appear to be in ${destination}, would you like to visit that site instead?</p><div class="modal-buttons"><button href="${destinationUrl}">Shop in ${destination}</button><button data-dismiss="modal">Stay Here</button></div>`
    document.getElementById('geo-modal').classList.add('open');
    document.cookie = `vr_geo=true; Secure`;
}
